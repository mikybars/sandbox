...
public class RunTareaAmbitoValidarCarenciaServiceImplTest {
  ...
  @InjectMocks
  private RunTareaAmbitoValidarCarenciaServiceImpl runTareaAmbitoValidarCarenciaServiceImpl;
  ...
  public void execute() {
    ...
    when(this.comisAsyncService.findCarencia(any(RunTareaDto.class), any(TareaAmbitoDto.class)))
        .thenReturn(cf);

    this.runTareaAmbitoValidarCarenciaServiceImpl.execute(runTareaDto, tareaAmbitoDto, tareaFaseAccionDto);
    ...
    verify(this.primaryTemporaryTableRepositoryCustom, timeout(1000).times(1))
        .createTempComisCarencia();
    verify(this.primaryTemporaryTableRepositoryCustom, timeout(1000).times(1))
        .insertTempComisCarencia(ArgumentMatchers.any(List.class));
    verify(this.primaryTemporaryTableRepositoryCustom, timeout(1000).times(1))
        .validateTempComisCarencia(ArgumentMatchers.any(TareaDto.class));
    verify(this.primaryTemporaryTableRepositoryCustom, timeout(1000).times(1))
        .deleteTempComisCarencia();
