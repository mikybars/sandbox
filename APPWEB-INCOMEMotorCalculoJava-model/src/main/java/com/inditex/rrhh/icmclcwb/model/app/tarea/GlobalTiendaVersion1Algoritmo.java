...
public class GlobalTiendaVersion1Algoritmo implements Algoritmo {
    ...
    @Override
    public Flux<Void> execute(RunTareaDto runTarea) {
        if (runTarea.getRunTareaCalcular().getEmpleado().size() >= algoritmoProperties.getMaxBatchSize()) {
            ...
            Flux.fromIterable(StreamUtils.partition(/* TODO Hay que lanzarlo por empleado y ordinal */runTarea
                    ...
                    .doOnNext(idsEmpleados -> tareaCalculoAlgoritmoGlobalTiendaRepository
                            .calcularByIdTareaAndIdsEmpleado(runTarea.getTarea().getId(), idsEmpleados))
            ...
            tareaCalculoAlgoritmoGlobalTiendaRepository.calcularByIdTarea(runTarea.getTarea().getId());
        }
        return Flux.empty();
    }
