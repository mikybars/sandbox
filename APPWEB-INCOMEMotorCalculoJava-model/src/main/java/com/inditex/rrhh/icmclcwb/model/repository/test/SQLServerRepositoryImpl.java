...
public class SQLServerRepositoryImpl implements SQLServerRepository {
    ...
    public void crearTemporalTest2() {
        ...
        jdbcTemplate.execute(
                "CREATE TABLE #TEST2 (_id bigint primary key identity, fecha datetime, IDPAIS bigint, IDCADENA bigint, IDTIENDA bigint, importeSinIVA float, importeConIVA float);");
    ...
    public void crearTemporalTest3() {
        ...
        jdbcTemplate.execute(
                "CREATE TABLE #TEST3 (_id bigint primary key identity, fecha datetime, idPais bigint, idCadena bigint, idTienda bigint, importeSinIVA float, importeConIVA float);");
...
//		.append(contenidoFichero).append("', 2));").toString();
        final String sql = new StringBuffer().append("INSERT INTO #TEST1 (log) VALUES ('").append(contenidoFichero)
                .append("');").toString();
    ...
    public List<VentaDto> getDatosXMLCompleto() {
        ...
        final String sql = new StringBuffer().append("SELECT A.*").append("FROM ").append("(SELECT ")
                ...
                .append(" p.value('(./importeConIVA/node())[1]', 'float') as importeConIVA").append(" FROM #TEST1 T1")
                .append(" CROSS APPLY log.nodes('/ventaTotalizadoResponse/ventaTotalizado') t(p)").append(") A")
                .append(" WHERE A.idTienda=224").toString();
    ...
    public List<VentaDto> getDatosJoinXMLCompleto() {
        ...
        String sql = new StringBuffer().append("INSERT INTO #TEST2 ")
                .append("(fecha, idPais, idCadena, idTienda, importeSinIVA, importeConIVA) ").append("SELECT ")
                .append(" p.value('(./fecha/node())[1]', 'datetime') as fecha,")
                .append(" p.value('(./pais/node())[1]', 'bigint') as idPais,")
                .append(" p.value('(./cadena/node())[1]', 'bigint') as idCadena,")
                .append(" p.value('(./tienda/node())[1]', 'bigint') as idTienda,")
                .append(" p.value('(./importeSinIVA/node())[1]', 'float') as importeSinIVA,")
                .append(" p.value('(./importeConIVA/node())[1]', 'float') as importeConIVA").append(" FROM #TEST1 T1")
                .append(" CROSS APPLY log.nodes('/ventaTotalizadoResponse/ventaTotalizado') t(p);").toString();
        jdbcTemplate.execute(sql);
        logger.info("Se inserta datos en temporal TEST2: OK : {}", getLogTiempo(time1));

        time1 = new java.util.Date();
        logger.info("Se inserta datos en temporal TEST3");
        sql = new StringBuffer().append("INSERT INTO #TEST3 ")
        ...
        sql = new StringBuffer()
                .append("SELECT t1.fecha, t1.idPais, t1.idCadena, t1.idTienda, t1.importeSinIVA, t1.importeConIVA ")
                .append("FROM #TEST2 t1 ").append("INNER JOIN #TEST3 t2 ").append(" ON t1.fecha=t2.fecha")
    ...
    public void insertarDatosXMLTrozos(final List<VentaDto> ventas) {
        java.util.Date time1 = new java.util.Date();
        logger.info("Se inserta datos en temporal TEST2");
        final String sql = new StringBuffer().append("INSERT INTO #TEST2 ")
                .append("(fecha, idPais, idCadena, idTienda, importeSinIVA, importeConIVA) ")
    ...
    public List<VentaDto> getDatosXMLTrozos() {
        ...
        final String sql = new StringBuffer().append("SELECT ")
                .append(" fecha, idPais, idCadena, idTienda, importeSinIVA, importeConIVA").append(" FROM #TEST2")
                .append(" WHERE idTienda=224").toString();
        final List<VentaDto> resultado = jdbcTemplate.query(sql, new BeanPropertyRowMapper(VentaDto.class));
    ...
    public List<VentaDto> getDatosJoinXMLTrozos() {
        ...
        java.util.Date time1 = new java.util.Date();
        logger.info("Se inserta datos en temporal TEST3");
        String sql = new StringBuffer().append("INSERT INTO #TEST3 ")
                .append("(fecha, idPais, idCadena, idTienda, importeSinIVA, importeConIVA) ")
                .append("SELECT fecha, idPais, idCadena, idTienda, importeSinIVA, importeConIVA")
                .append(" FROM #TEST2;").toString();
        jdbcTemplate.execute(sql);
    ...
    private void borrarTemporal(final String nombre) {
        jdbcTemplate.execute(new StringBuilder("IF OBJECT_ID('tempdb..#").append(nombre)
                .append("') IS NOT NULL BEGIN  DROP TABLE #").append(nombre).append(" END;").toString());
    }
