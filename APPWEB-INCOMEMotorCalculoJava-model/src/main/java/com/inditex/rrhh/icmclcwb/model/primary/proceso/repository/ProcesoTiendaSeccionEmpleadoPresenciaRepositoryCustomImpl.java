...
@Repository
public class ProcesoTiendaSeccionEmpleadoPresenciaRepositoryCustomImpl
        extends JdbcBatchPrimaryRepositoryAbstract<ProcesoTiendaSeccionEmpleadoPresencia>
        implements ProcesoTiendaSeccionEmpleadoPresenciaRepositoryCustom {

    @Autowired
    @Qualifier("presenciasProperties")
    private Map<String, PtrPropertiesDto> presenciasProperties;

    @Value("#{primaryQuery['ProcesoTiendaSeccionEmpleadoPresenciaRepositoryCustom.save']}")
    private String sqlSave;

    @Override
    public List<ProcesoTiendaSeccionEmpleadoPresencia> save(final List<ProcesoTiendaSeccionEmpleadoPresencia> src) {
        return saveJdbcBatchList(src, sqlSave, presenciasProperties.get(PtrConstants.PRESENCIA_DETALLE).getFilter().getMaxBatchSize());
    }

    @Override
    public void setParameters(PreparedStatement pstmt, ProcesoTiendaSeccionEmpleadoPresencia entity)
            throws SQLException {
        ...
        pstmt.setLong(7, entity.getProceso().getId());
    }
