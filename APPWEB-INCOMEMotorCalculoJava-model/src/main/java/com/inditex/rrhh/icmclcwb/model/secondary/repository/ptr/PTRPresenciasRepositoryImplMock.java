...
public class PTRPresenciasRepositoryImplMock implements PTRPresenciasRepositoryMock{
	...
	private String consultaPresenciaDetalleEspana = "SELECT TOP 1[ID],[TIPO],[TIENDA],[FECHA],[SECCION],[PERSONA],[HORAS],[CCL_ID_ORIGEN] FROM [dbo].[PRESENCIAS_HORARIOS] P INNER JOIN M4CCL_ORGANIZACION_PAIS OP ON  P.ID_ORGANIZATION=OP.ID_ORGANIZATION AND OP.CCL_ID_ORIGEN=11 WHERE TIENDA = ? AND FECHA >= ? AND FECHA <= ? AND ERROR = 'OK'";
	private String consultaTiposHorasEspana ="SELECT TOP 100 [ID],[TIPO],[TIENDA],[FECHA],[SECCION],[PERSONA],[HORAS],[CCL_ID_ORIGEN] FROM [dbo].[PRESENCIAS_HORARIOS] P INNER JOIN M4CCL_ORGANIZACION_PAIS OP ON  P.ID_ORGANIZATION=OP.ID_ORGANIZATION AND OP.CCL_ID_ORIGEN=11 WHERE TIPO = ? AND ERROR = 'OK'";
