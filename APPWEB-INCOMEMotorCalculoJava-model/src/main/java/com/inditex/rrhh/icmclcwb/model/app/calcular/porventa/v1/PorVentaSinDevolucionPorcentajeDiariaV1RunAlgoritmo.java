...
public class PorVentaSinDevolucionPorcentajeDiariaV1RunAlgoritmo implements RunAlgoritmo {
    ...
    public void execute(RunTareaDto runTarea, AlgoritmoDto algoritmo) {
        Flux.fromIterable(StreamUtils.partition(
            tareaCalculoAlgoritmoPorVentaSinDevolucionPorcentajeDiariaV1RepositoryCustom.ids(algoritmo, runTarea.getTarea()),
            runAlgoritmoProperties.getBatchSize())).parallel().runOn(ItxSchedulers.elastic()).map(personas -> {
            log.info("Inicio :: PorVentaSinDevolucionPorcentajeDiariaV1RunAlgoritmo :: Personas: {}", personas.size());
            try {
                tareaCalculoAlgoritmoPorVentaSinDevolucionPorcentajeDiariaV1RepositoryCustom.calcular(algoritmo,
                    runTarea.getTarea(), personas);
            } catch (Exception e) {
                log.error("PorVentaSinDevolucionPorcentajeDiariaV1RunAlgoritmo :: KO :: Personas: {}", personas.size(), e);
    ...
    public String getSqlCalcular(AlgoritmoDto algoritmo) {
        return tareaCalculoAlgoritmoPorVentaSinDevolucionPorcentajeDiariaV1RepositoryCustom.getSqlCalcular(algoritmo);
    }
