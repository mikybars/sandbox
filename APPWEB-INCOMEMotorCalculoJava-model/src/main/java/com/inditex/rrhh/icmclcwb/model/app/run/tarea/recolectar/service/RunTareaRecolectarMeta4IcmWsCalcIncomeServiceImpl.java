            ...
            for (TareaAmbitoDto tareaAmbito : tarea.getAmbito()) {
                ...
                if (CollectionUtils.isNotEmpty(tarea.getPersona())) {
                    for (List<TareaAmbitoPersonaDto> iter : StreamUtils.partition(tarea.getPersona().stream()
                            ...
                            .collect(Collectors.toList()), meta4Properties.get(Meta4Constants.SEARCH_EMPLEADOS).getFilter().getMaxPageSize())) {
                        SearchEmpleadosRequestDto searchEmpleadosRequest = new SearchEmpleadosRequestDto();
                        searchEmpleadosRequest.setPage(meta4Properties.get(Meta4Constants.SEARCH_EMPLEADOS).getPage());
                ...
                do {
                    ...
                    if (CollectionUtils.isNotEmpty(dataSearchTiendas)) {
                        AsyncUtils.checkAsyncAvaliable(cfPersist, meta4Properties.get(Meta4Constants.SEARCH_TIENDAS).getFilter().getMaxPersistenceSize());
            ...
            for (TareaAmbitoDto tareaAmbito : tarea.getAmbito()) {
                if (CollectionUtils.isNotEmpty(tarea.getLocalizacion())) {
                    ...
                    for (List<GenericFilterParametersDto> iter : StreamUtils.partition(
                        ...
                        tiendasEmpleadoRequest.setPage(meta4Properties.get(Meta4Constants.TIENDAS_EMPLEADO).getPage());
...
//                }
        ...
        try {
            ...
            for (TareaAmbitoDto tareaAmbito : tarea.getAmbito()) {
                if (CollectionUtils.isEmpty(tarea.getPersona())) {
                    ...
                    empleadosRequest.setPage(meta4Properties.get(Meta4Constants.EMPLEADOS).getPage());
    ...
    public void personaByRunTareaAndTareaAmbito(@Valid final RunTareaDto runTarea,
        ...
        try {
            ...
            do {
                ...
                AsyncUtils.exceptionally(cfData, cf);
                List<GenericEmpleadoResultItemDto> data = AsyncUtils.get(cfData);
                AsyncUtils.checkAsyncAvaliable(cfPersist, meta4Properties.get(Meta4Constants.SEARCH_EMPLEADOS).getFilter().getMaxPersistenceSize());
                CompletableFuture<Void> cfSave = tareaPersonaHistoricoAsyncService
                        .saveGenericEmpleadoResultItemDto(data, tarea);
                AsyncUtils.exceptionally(cfSave, cf, cfPersist);
    ...
    public void empleadosPresenciaByRunTareaAndTareaAmbito(@NotNull @Valid final RunTareaDto runTarea,
        ...
        try {
            ...
            AsyncUtils.exceptionally(cfData, cf);
            List<GenericEmpleadoResultItemDto> data = AsyncUtils.get(cfData);
            AsyncUtils.checkAsyncAvaliable(cfPersist, meta4Properties.get(Meta4Constants.EMPLEADOS_PRESENCIA).getFilter().getMaxPersistenceSize());
            CompletableFuture<Void> cfSave = tareaLocalizacionPersonaPresenciaManualAsyncService.saveGenericEmpleadoResultItemDto(data, tarea);
            AsyncUtils.exceptionally(cfSave, cf, cfPersist);
            AsyncUtils.waitAllOfIsOk(cf);
        } catch (Exception e) {
            AsyncUtils.cancel(cf);
    ...
    public void presenciaManualByRunTareaAndTareaAmbito(@NotNull @Valid final RunTareaDto runTarea,
        ...
        try {
            ...
            AsyncUtils.exceptionally(cfData, cf);
            List<GenericEmpleadoResultItemDto> data = AsyncUtils.get(cfData);
            AsyncUtils.checkAsyncAvaliable(cfPersist, meta4Properties.get(Meta4Constants.PRESENCIA_MANUAL).getFilter().getMaxPersistenceSize());
           //TODO: Persistir
//                AsyncUtils.exceptionally(cfSave, cf, cfPersist);
            AsyncUtils.waitAllOfIsOk(cf);
        } catch (Exception e) {
...
//                AsyncUtils.exceptionally(cfSave, cf, cfPersist);
    ...
    public void condicionPersonaByRunTareaAndTareaAmbito(@NotNull @Valid final RunTareaDto runTarea,
        ...
        try {
            final TrabajoDto trabajo = runTarea.getTrabajo();
            final TareaDto tarea = runTarea.getTarea();
            for (List<IdPersonaDto> iter : StreamUtils.partition(tareaPersonaHistoricoervice
                ...
                AsyncUtils.exceptionally(cfData, cf);
                List<GenericEmpleadoResultItemDto> data = AsyncUtils.get(cfData);
                AsyncUtils.checkAsyncAvaliable(cfPersist, meta4Properties.get(Meta4Constants.COMISION_EMPLEADO).getFilter().getMaxPersistenceSize());
                CompletableFuture<Void> cfSave = tareaEmpleadoEstructuraAsyncService
                        .saveGenericEmpleadoResultItemDto(data, tarea);
                AsyncUtils.exceptionally(cfSave, cf, cfPersist);
                AsyncUtils.waitAllOfIsOk(cf);
    ...
    private void localizacionHistoricoByRunTareaAndTareaAmbito(@Valid RunTareaDto runTarea,
        ...
        List<IdPersonaDto> idsPersona = tareaPersonaHistoricoervice.findIdPersonaByIdTareaAndIdOrigen(tarea.getId(), tareaAmbito.getIdOrigen());
        try {
            SearchTiendasRequestDto searchTiendasRequest = new SearchTiendasRequestDto();
            searchTiendasRequest.setPage(meta4Properties.get(Meta4Constants.SEARCH_TIENDAS).getPage());
            searchTiendasRequest.setData(tareaMapper
