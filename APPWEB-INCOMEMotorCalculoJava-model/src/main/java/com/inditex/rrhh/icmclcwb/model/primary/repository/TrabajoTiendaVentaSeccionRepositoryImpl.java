...
public class TrabajoTiendaVentaSeccionRepositoryImpl implements TrabajoTiendaVentaSeccionRepository {
    ...
    private static final String SAVE = "MERGE INTO DESARROLLO_RRHH.INCOME_TRABAJO_TIENDA_VENTA_SECCION" + " USING ("
            + " SELECT FECHA, " + " ID_TIENDA, " + " COALESCE(MAX(Q1),0) AS IMPORTE_1, "
            + " COALESCE(MAX(Q2),0) AS IMPORTE_2, " + " COALESCE(MAX(Q3),0) AS IMPORTE_3, " + " ID_TRABAJO " + " FROM( "
            ...
            + " FROM DESARROLLO_RRHH.INCOME_TRABAJO_TIENDA_SECCION_VENTA A " + " WHERE A.ID_TRABAJO = ? "
