...
public class PtrPresenciasMockRepositoryImpl implements PtrPresenciasMockRepository {
	...
	public List<PresenciaTotalTiendaMock> findPresenciasTotalTienda(PresenciasTotalTiendaRequestDto dto) {
		...
		query.append(
				") P INNER JOIN M4CCL_ORGANIZACION_PAIS OP WITH (NOLOCK) ON P.ID_ORGANIZATION = OP.ID_ORGANIZATION AND P.REG_NUM = 1 WHERE 1 = 1 " + origen);
...
**/
	...
	public List<PresenciaTotalTiendaSeccionMock> findPresenciasTotalTiendaSeccion(
		...
		StringBuilder query = new StringBuilder();
		query.append( "SELECT P.TIENDA AS 'ID_TIENDA', P.FECHA AS 'FECHA', P.SECCION AS 'ID_SECCION', SUM((CAST(P.HORAS AS int) * 60) + "
		...
		query.append ( " INNER JOIN STD_WORK_LOCATION SW WITH (NOLOCK) ON SW.CCL_ID_COD_ORIGEN = CAST(PT.TIENDA AS nvarchar) INNER "
