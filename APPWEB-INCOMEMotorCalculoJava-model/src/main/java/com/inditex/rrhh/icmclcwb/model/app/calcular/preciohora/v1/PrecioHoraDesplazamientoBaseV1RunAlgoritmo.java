...
public class PrecioHoraDesplazamientoBaseV1RunAlgoritmo implements RunAlgoritmo {
    ...
    public void execute(RunTareaDto runTarea, AlgoritmoDto algoritmo) {
        Flux.fromIterable(StreamUtils.partition(
                ...
                runAlgoritmoProperties.getBatchSize())).parallel().runOn(ItxSchedulers.elastic()).map(personas -> {
                    ...
                    try {
                        tareaCalculoAlgoritmoPrecioHoraDesplazamientoBaseV1RepositoryCustom.calcular(algoritmo,
                                runTarea.getTarea(), personas);
                    } catch (Exception e) {
                        log.error("PrecioHoraDesplazamientoBaseV1RunAlgoritmo :: KO :: Personas: {}", personas.size(), e);
                        tareaCalculoPersonaService.updateWithEstadoAndidPersona(personas, runTarea,
                                EstadoTareaCalculoPersonaEnum.KO.getDto());
                    }
                    log.info("Fin :: PrecioHoraDesplazamientoBaseV1RunAlgoritmo :: Personas: {}", personas.size());
                    return Flux.empty();
    ...
    public String getSqlCalcular(AlgoritmoDto algoritmo) {
        return tareaCalculoAlgoritmoPrecioHoraDesplazamientoBaseV1RepositoryCustom.getSqlCalcular(algoritmo);
    }
