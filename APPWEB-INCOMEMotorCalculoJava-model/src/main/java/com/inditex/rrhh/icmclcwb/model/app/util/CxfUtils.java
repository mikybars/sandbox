...
public class CxfUtils {
    ...
    public static void putCookie(final Object service, final String jSessionID) {
        Client cl = ClientProxy.getClient(service);
        HTTPConduit http = (HTTPConduit) cl.getConduit();
        http.getClient().setCookie(new StringBuilder("JSESSIONID=").append(jSessionID).toString());
    }
...
//        httpConduitService.getClient().setCookie(httpConduitLogin.getClient().getCookie());
        httpConduitService.getCookies().clear();
        httpConduitService.getCookies().putAll(httpConduitLogin.getCookies());
//        replaceRequestHeaders(service, CxfUtils.mapCookie(CxfUtils.getSetCookie(getResponseHeaders(serviceLogin))));
    }
