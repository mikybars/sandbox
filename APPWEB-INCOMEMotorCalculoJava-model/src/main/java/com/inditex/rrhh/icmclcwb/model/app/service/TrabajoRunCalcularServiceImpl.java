...
public class TrabajoRunCalcularServiceImpl implements TrabajoRunCalcularService {
    ...
    public TrabajoRunDto run(@Valid final TrabajoRunDto trabajoRun) throws Exception {
        ...
        if (EstadoTrabajoEnum.PENDIENTE_CALCULO.getId().equals(trabajo.getEstado().getId())) {
            ...
            // comision, etc...
            trabajoRun.getTrabajoRunCalcular().getTiposCalculo()
            ...
            Flux.fromIterable(trabajoRun.getTrabajoRunCalcular().getTiposCalculo()).log().parallel()
                    .runOn(Schedulers.parallel())
                    .doOnNext(tipo -> calculoAlgoritmoFactory.getAlgoritmo(TipoCalculoEnum.of(tipo).getType())
                            .execute(trabajoRun).onErrorResume(error -> {
