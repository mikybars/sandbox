...
public abstract class JobMapperDecorator implements JobMapper {
	...
	public JobDto scheduleDtoToJobDto(ScheduleDto src) {
		...
		result.setFechaInicioPeriodo(periodo.with(TemporalAdjusters.firstDayOfMonth()));
		result.setFechaFinPeriodo(periodo.with(TemporalAdjusters.firstDayOfNextMonth()));
		return result;
	}
