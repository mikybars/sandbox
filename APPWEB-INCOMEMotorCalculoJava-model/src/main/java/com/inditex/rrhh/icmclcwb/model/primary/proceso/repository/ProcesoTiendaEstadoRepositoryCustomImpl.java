...
@Repository
public class ProcesoTiendaEstadoRepositoryCustomImpl extends JdbcBatchPrimaryRepositoryAbstract<ProcesoTiendaEstado>
        implements ProcesoTiendaEstadoRepositoryCustom {
    ...
    @Value("#{primaryQuery['ProcesoTiendaEstadoRepositoryCustom.customFindByIdTiendaNotExists']}")
    private String sqlCustomFindByIdTiendaNotExists;

    @Value("#{primaryQuery['ProcesoTiendaEstadoRepositoryCustom.save']}")
    private String sqlSave;

    @Override
    public List<ProcesoTiendaEstado> save(final List<ProcesoTiendaEstado> src) {
        return saveJdbcBatchList(src, sqlSave, searchTiendasDto.getFilter().getMaxBatchSize());
    }

    @Override
    public void setParameters(PreparedStatement pstmt, ProcesoTiendaEstado entity) throws SQLException {
