...
public class GTCalculoRepositoryImpl  implements GTCalculoRepository{
	...
	private StringBuilder queryCalculoGT() {
		...
		//TODO Revision si viene porcentaje 0 hay minutos 0
		append(" ,(TIENDA_VENTA.IMPORTE_1) * CAST ( CASE WHEN  ESTRUCTURA.PORCENTAJE IS NULL THEN ESTRUCTURA.PORCENTAJE_1 ELSE ESTRUCTURA.PORCENTAJE END AS FLOAT ) * ( CAST (EMPLEADO_PRESENCIA.MINUTOS_1 AS FLOAT) / TIENDA_PRESENCIA.MINUTOS_1) AS IMPORTE_1 ").   
		append(" ,(TIENDA_VENTA.IMPORTE_2) * CAST ( CASE WHEN  ESTRUCTURA.PORCENTAJE IS NULL THEN ESTRUCTURA.PORCENTAJE_2 ELSE ESTRUCTURA.PORCENTAJE END AS FLOAT ) * ( CAST (EMPLEADO_PRESENCIA.MINUTOS_2 AS FLOAT) / TIENDA_PRESENCIA.MINUTOS_2) AS IMPORTE_2  ").  
		append(" ,(TIENDA_VENTA.IMPORTE_3) * CAST ( CASE WHEN  ESTRUCTURA.PORCENTAJE IS NULL THEN ESTRUCTURA.PORCENTAJE_3 ELSE ESTRUCTURA.PORCENTAJE END AS FLOAT ) * ( CAST (EMPLEADO_PRESENCIA.MINUTOS_3 AS FLOAT) / TIENDA_PRESENCIA.MINUTOS_3) AS IMPORTE_3  ").
		...
		append("  INNER JOIN DESARROLLO_RRHH.INCOME_TRABAJO_TIPO_HORA AS TIPO_HORA  ").
