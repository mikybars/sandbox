...
public class TareaRecolectarMeta4IcmWsCalcIncomeServiceImpl implements TareaRecolectarMeta4IcmWsCalcIncomeService {
    ...
    public void tiendasHistorico(@Valid final TareaDto tarea,
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(tarea.getPersona())) {
                for (List<TareaAmbitoPersonaDto> iter : StreamUtils.partition(tarea.getPersona(),
                ...
                tiendas.addAll(
                        tarea.getLocalizacion().stream().map(TareaAmbitoLocalizacionDto::getIdLocalizacion).collect(Collectors.toSet()));
            }
    ...
    public void condicionesEmpleados(@Valid final TareaDto tarea,
        ...
        try {
            List<CompletableFuture<?>> cfPersist = new ArrayList<>();
            for (List<String> iter : StreamUtils.partition(runTareaRecolectarBloque.getEmpleado().stream().map(TareaEmpleadoEstadoDto::getIdEmpleado).collect(Collectors.toList()),
        ...
        try {
            if (CollectionUtils.isEmpty(tarea.getPersona())) {
                ...
                if (CollectionUtils.isNotEmpty(tarea.getLocalizacion())) {
                    ...
                    tarea.getLocalizacion().stream().forEach(item -> {
                        if (StringUtils.isNotBlank(item.getIdLocalizacion())) {
                            empleadosRequestItem.add(
                                    GenericFilterParametersDto.builder().idLugarTrabajo(item.getIdLocalizacion()).build());
                        } else {
                            log.warn(
