...
@Service
@Validated
public class TareaRecolectarMeta4IcmWsCalcIncomeServiceImpl implements TareaRecolectarMeta4IcmWsCalcIncomeService {
    ...
    @TareaAuditoria
    @Override
    public void tiendasHistorico(@Valid final TareaDto tarea,
            @Valid final RunTareaRecolectarBloqueDto runTareaRecolectarBloque) {
        List<CompletableFuture<?>> cf = new ArrayList<>();
        try {
            Set<String> tiendas = new HashSet<>();
            if (CollectionUtils.isNotEmpty(tarea.getEmpleados())) {
                for (List<TareaEmpleadoDto> iter : StreamUtils.partition(tarea.getEmpleados(),
                    ...
                    searchEmpleadosRequest.setData(tareaMapper.tareaDtoToGenericFilterDto(tarea));
                        ...
                        if (CollectionUtils.isNotEmpty(dataSearchEmpleados)) {
                            ...
                            runTareaRecolectarBloque.getEmpleado()
                                    .addAll(tareaEmpleadoEstadoMapper
                                            .genericEmpleadoResultItemDtoToTareaEmpleadoEstadoDto(dataSearchEmpleados,
                                                    tarea));
                        }
            ...
            } else if (CollectionUtils.isNotEmpty(tarea.getTiendas())) {
                tiendas.addAll(
                        tarea.getTiendas().stream().map(TareaTiendaDto::getIdTienda).collect(Collectors.toSet()));
            }
            ...
            searchTiendasRequest.setData(tareaMapper.tareaDtoToGenericFilterDto(tarea));
            ...
            do {
                ...
                if (CollectionUtils.isNotEmpty(dataSearchTiendas)) {
                    List<TareaTiendaHistoricoDto> tareaTiendaHistorico = tareaTiendaHistoricoMapper
                            .genericTiendaResultItemDtoToTareaTiendaHistoricoDto(dataSearchTiendas);
                    if (CollectionUtils.isNotEmpty(tareaTiendaHistorico)) {
                        AsyncUtils.checkAsyncAvaliable(cfPersist, searchTiendasDto.getFilter().getMaxPersistenceSize());
                        CompletableFuture<Void> cfSave = tareaTiendaHistoricoAsyncService.save(tareaTiendaHistorico,
                                tarea);
                        AsyncUtils.exceptionally(cfSave, cf, cfPersist);
                    }
    ...
    @TareaAuditoria
    @Override
    public void tiendasPresencia(@Valid final TareaDto tarea,
            @Valid final RunTareaRecolectarBloqueDto runTareaRecolectarBloque) {
        List<CompletableFuture<?>> cf = new ArrayList<>();
        try {
            List<CompletableFuture<?>> cfPersist = new ArrayList<>();
    ...
    @TareaAuditoria
    @Override
    public void tiendasEmpleadoHistorico(@Valid TareaDto tarea,
            @Valid final RunTareaRecolectarBloqueDto runTareaRecolectarBloque) {
        List<CompletableFuture<?>> cf = new ArrayList<>();
        try {
            if (CollectionUtils.isNotEmpty(tarea.getTiendas())) {
                ...
                for (List<GenericFilterParametersDto> iter : StreamUtils
                    ...
                    tiendasEmpleadoRequest.setData(tareaMapper.tareaDtoToGenericFilterDto(tarea));
    ...
    @TareaAuditoria
    @Override
    public void condicionesEmpleados(@Valid final TareaDto tarea,
            @Valid final RunTareaRecolectarBloqueDto runTareaRecolectarBloque) {
        List<CompletableFuture<?>> cf = new ArrayList<>();
        try {
            List<CompletableFuture<?>> cfPersist = new ArrayList<>();
            for (List<String> iter : StreamUtils.partition(runTareaRecolectarBloque.getEmpleadoLocal(),
                ...
                comisionEmpleadoRequest.setData(tareaMapper.tareaDtoToGenericFilterDto(tarea));
                ...
                if (CollectionUtils.isNotEmpty(data)) {
                    List<TareaEmpleadoEstructuraDto> tareaEmpleadoEstructura = tareaEmpleadoEstructuraMapper
                            .genericEmpleadoResultItemDtoToTareaEmpleadoEstructuraDto(data, tarea);
                    if (CollectionUtils.isNotEmpty(tareaEmpleadoEstructura)) {
                        AsyncUtils.checkAsyncAvaliable(cfPersist,
                                getComisionEmpleadoDto.getFilter().getMaxPersistenceSize());
                        CompletableFuture<Void> cfSave = tareaEmpleadoEstructuraAsyncService
                                .save(tareaEmpleadoEstructura, tarea);
                        AsyncUtils.exceptionally(cfSave, cf, cfPersist);
                    }
                }
            }
                ...
                for (int x = 1; x < 3; x++) {
                    for (int y = 1; y < 3; y++) {
                        ...
                        mockData.setFechaInicio(
                                Date.from(tarea.getFechaInicioPeriodo().atZone(ZoneId.systemDefault()).toInstant()));
    ...
    @TareaAuditoria
    @Override
    public void tiendasComisionable(@Valid final TareaDto tarea,
            @Valid final RunTareaRecolectarBloqueDto runTareaRecolectarBloque) {
        List<CompletableFuture<?>> cf = new ArrayList<>();
        try {
            List<CompletableFuture<?>> cfPersist = new ArrayList<>();
            for (List<String> iter : StreamUtils.partition(runTareaRecolectarBloque.getTiendaMeta4(),
                ...
                tiendasRequest.setData(tareaMapper.tareaDtoToGenericFilterDto(tarea));
    ...
    @TareaAuditoria
    @Override
    public void empleadosTienda(@Valid final TareaDto tarea,
            @Valid final RunTareaRecolectarBloqueDto runTareaRecolectarBloque) {
        List<CompletableFuture<?>> cf = new ArrayList<>();
        try {
            if (CollectionUtils.isEmpty(tarea.getEmpleados())) {
                EmpleadosRequestDto empleadosRequest = new EmpleadosRequestDto();
                empleadosRequest.setPage(getEmpleadosDto.getPage());
                empleadosRequest.setData(tareaMapper.tareaDtoToGenericFilterDto(tarea));
                if (CollectionUtils.isNotEmpty(tarea.getTiendas())) {
                    Set<GenericFilterParametersDto> empleadosRequestItem = new HashSet<>();
                    tarea.getTiendas().stream().forEach(item -> {
                    ...
                    if (CollectionUtils.isNotEmpty(data)) {
                        data.stream().forEach(item -> {
                            if (StringUtils.isNotBlank(item.getIdEmpleado())
                                ...
                                log.warn(
                                        "TareaRecolectarMeta4IcmWsCalcIncomeServiceImpl :: meta4IcmWsCalcIncomeSessionAsyncService.searchEmpleados() :: isBlank() :: {}",
                                        item);
                            }
                        });
                    }
