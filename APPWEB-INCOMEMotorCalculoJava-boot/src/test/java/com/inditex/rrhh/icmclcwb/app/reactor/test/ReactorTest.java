...
public class ReactorTest {
    ...
    public void ReactorSimpleTest1() throws InterruptedException {
        ...
        ParallelFlux<Object> parallelFlux = empleados.log().parallel().runOn(Schedulers.parallel()).map((t) -> {
            return tiempo(t);
        });
    ...
    public void ReactorSimpleTest2() {
        ...
        CountDownLatch latch = new CountDownLatch(1);
        empleados.log().parallel().runOn(Schedulers.parallel()).doOnNext(n -> {
        ...
        }).doAfterTerminate(latch::countDown).doOnError(error -> System.out.println("Error: " + error)).subscribe(
                m -> System.out
                        .println("Subscribe recibido - " + m + " en thread: " + Thread.currentThread().getName()),
                error -> System.out.println("error subscribe:" + error), null);
