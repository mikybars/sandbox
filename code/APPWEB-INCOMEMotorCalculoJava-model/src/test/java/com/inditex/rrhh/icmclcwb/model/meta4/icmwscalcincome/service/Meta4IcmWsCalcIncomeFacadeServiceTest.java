...
class Meta4IcmWsCalcIncomeFacadeServiceTest {
  ...
  class GetFlagCalcula {
    ...
    @Test
    void whenInvokedExpectDelegateToSoapResult() {
      when(soapService.getFlagCalcula(request)).thenReturn(response);
  ...
  class GetTiendasOnline {
    ...
    void whenInvokedExpectDispatcherResultReturned() {
      when(migrationDispatcher.dispatch(eq("getTiendasOnline"), any(), any())).thenReturn(response);

      TiendaOnlineResponseDto result = service.getTiendasOnline(request);

      assertThat(result).isSameAs(response);
      verify(migrationDispatcher, times(1)).dispatch(eq("getTiendasOnline"), any(), any());
    }
  ...
  class GetEmpresa {
    ...
    @Test
    void whenInvokedExpectDelegateToSoapResult() {
      when(soapService.getEmpresa(request)).thenReturn(response);
  ...
  class GetEmpleadosDesplazamiento {
    ...
    @Test
    void whenInvokedExpectDelegateToSoapResult() {
      when(soapService.getEmpleadosDesplazamiento(request)).thenReturn(response);
