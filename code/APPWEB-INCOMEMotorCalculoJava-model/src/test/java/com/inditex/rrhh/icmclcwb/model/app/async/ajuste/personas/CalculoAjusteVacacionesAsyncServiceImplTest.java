...
class CalculoAjusteVacacionesAsyncServiceImplTest {

  @Mock
  private CalculoAjusteVacacionesService calculoAjusteVacacionesService;

  @InjectMocks
  private CalculoAjusteVacacionesAsyncServiceImpl calculoAjusteVacacionesAsyncService;
  ...
  void ajustarTest() {
    ...
    final List<IdPersonaLocalDto> personas = Collections.singletonList(
        IdPersonaLocalDto
            ...
            .build());
    final CompletableFuture<Void> cf = this.calculoAjusteVacacionesAsyncService.ajustar(algoritmo, tarea, personas);
    verify(this.calculoAjusteVacacionesService, times(1)).ajustar(algoritmo, tarea, personas);
    assertNotNull(cf);
  }
