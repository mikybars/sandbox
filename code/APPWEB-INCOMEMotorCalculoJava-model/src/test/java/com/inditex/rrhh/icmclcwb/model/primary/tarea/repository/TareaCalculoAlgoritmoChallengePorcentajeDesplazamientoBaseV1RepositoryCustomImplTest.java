...
public class TareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustomImplTest {
  ...
  @InjectMocks
  private TareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustomImpl tareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustom;

  @BeforeEach
  public void setup() throws IllegalAccessException {
    FieldUtils.writeField(this.tareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustom, "sqlCalcular",
        SQL_CALCULAR, true);
    FieldUtils.writeField(this.tareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustom, "sqlCalcularBase",
        SQL_BASE, true);
  }
  ...
  public void idsTest() {
    ...
    final List<IdPersonaLocalDto> ids = this.tareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustom.ids(
        algoritmo,
        tarea);
  ...
  public void getMapValuesTest() {
    ...
    final Map<String, Object> result = this.tareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustom
        .getMapValues(algoritmo, tarea, persona1);

    /*
     * // parametros de la consulta: idSeccion, activo, tipoDatoLocalizacionVentaSeccion, excluido calculo, // tipoDatoPersonaPresencia,
     * idAlgoritmo, idTarea, cclIdPerson, stdOrHrPeriod, comisionable, calcula // tipoDatoLocalizacionPersonaPresencia
     * verify(this.tipoDatoService).findTipoDatoByTipoGrupoDato(TipoGrupoDatoEnum.VENTA_LOCALIZACION_SECCION.getId()); assertEquals(16,
     * result.size()); // activo assertTrue(result.containsKey(SQL_PARAM_ACTIVO)); assertEquals(SQL_VALUE_BOOLEAN_TRUE,
     * result.get(SQL_PARAM_ACTIVO)); // tipoDatoLocalizacionVentaSeccion
     * assertTrue(result.containsKey(SQL_PARAM_TIPO_DATO_LOCALIZACION_VENTA_SECCION)); assertEquals(Arrays.asList(1011),
     ...
     * result.get(SQL_PARAM_TIPO_DATO_PERSONA_PRESENCIA)); // tipoDatoLocalizacionPersonaPresencia
     * assertTrue(result.containsKey(SQL_PARAM_TIPO_DATO_LOCALIZACION_PERSONA_PRESENCIA)); assertEquals(
     ...
     * result.get(SQL_PARAM_TIPO_DATO_LOCALIZACION_PERSONA_PRESENCIA)); // idAlgoritmo
     * assertTrue(result.containsKey(SQL_PARAM_ID_ALGORITMO)); assertEquals(algoritmo.getId(), result.get(SQL_PARAM_ID_ALGORITMO)); //
     * idTarea assertTrue(result.containsKey(SQL_PARAM_ID_TAREA)); assertEquals(tarea.getId(), result.get(SQL_PARAM_ID_TAREA)); //
     * cclIdPerson assertTrue(result.containsKey(SQL_PARAM_CCL_ID_PERSON)); assertEquals(persona1.getIdPersonaLocal(),
     * result.get(SQL_PARAM_CCL_ID_PERSON)); // stdOrHrPeriod assertTrue(result.containsKey(SQL_PARAM_STD_OR_HR_PERIOD));
     * assertEquals(persona1.getStdOrHrPeriod(), result.get(SQL_PARAM_STD_OR_HR_PERIOD)); // comisionable
     * assertTrue(result.containsKey(SQL_PARAM_COMISIONABLE)); assertEquals(SQL_VALUE_BOOLEAN_TRUE, result.get(SQL_PARAM_COMISIONABLE)); //
     * calcula assertTrue(result.containsKey(SQL_PARAM_CALCULA)); assertEquals(SQL_VALUE_BOOLEAN_TRUE, result.get(SQL_PARAM_CALCULA)); //
     * excluido calculo assertTrue(result.containsKey(SQL_PARAM_EXCLUIDO_CALCULO)); assertEquals(SQL_VALUE_BOOLEAN_FALSE,
     * result.get(SQL_PARAM_EXCLUIDO_CALCULO)); // tipocomision assertTrue(result.containsKey(SQL_PARAM_IDS_TIPOS_COMISION));
     * assertEquals(Arrays.asList("001", "002", "003"), result.get(SQL_PARAM_IDS_TIPOS_COMISION)); // tipocalculo
     * assertTrue(result.containsKey(SQL_PARAM_IDS_TIPOS_CALCULO)); assertEquals(Arrays.asList("011", "012"),
     * result.get(SQL_PARAM_IDS_TIPOS_CALCULO)); // esDesplazamiento assertTrue(result.containsKey(SQL_PARAM_ES_DESPLAZAMIENTO));
     * assertEquals(SQL_VALUE_BOOLEAN_TRUE, result.get(SQL_PARAM_ES_DESPLAZAMIENTO)); // esDesplazamientoBase
     * assertTrue(result.containsKey(SQL_PARAM_ES_DESPLAZAMIENTO_BASE)); assertEquals(SQL_VALUE_BOOLEAN_FALSE,
  ...
  public void calcularTest() {
    ...
    this.tareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustom.calcular(algoritmo, tarea, personas);
  ...
  public void getSqlCalcularTest() {
    ...
    final String result = this.tareaCalculoAlgoritmoChallengePorcentajeDesplazamientoBaseV1RepositoryCustom
